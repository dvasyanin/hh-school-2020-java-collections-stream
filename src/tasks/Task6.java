...
public class Task6 implements Task {
    ...
    private Set<String> getPersonDescriptions(Collection<Person> persons,
        ...
        // нужна мапа для поиска региона по его id
        Map<Integer, String> areasNamesIds = areas.stream()
                .collect(Collectors.toMap(Area::getId, Area::getName));

        // для каждой персоны получаем стрим из айдишников регионов
        // на каждом найденном айдишнике получаем имя персоны и имя региона
        return persons.stream()
                .flatMap(person -> personAreaIds.get(person.getId()).stream()
                        .map(areaId -> person.getFirstName() + " - " + areasNamesIds.get(areaId)))
